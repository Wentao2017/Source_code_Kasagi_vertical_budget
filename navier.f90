...
if (itype.eq.6) then
   ...
   do j=1,xsize(2)
      ...
      do i=1,xsize(1)
         ...
         bxy1(j,k)=0.
         bxz1(j,k)=0.
!         ! Temporary storing y-derivative of ux1, uy1 and phi1 in uz1...   !Guo
!         uz1(i,j,1) = - twopi*mysx*mysy ! dyux                             !Guo
!         uz1(i,j,2) = - twopi*mycx*mycy ! dyuy                             !Guo
!         uz1(i,j,3) = twopi*mysx*mycy ! dyphi                              !Guo
      enddo
   enddo
   enddo   
endif
...
! Exact time history for the Taylor-Green benchmark    !Guo
!if (itype.eq.6) then                                   !Guo 
!   ! Explicit AB2 time-scheme                          !Guo
!   !                                                   !Guo 
!   ! Scalar                                            !Guo
!   tx=dt*(twopi**2)*xnu/(2.d0*sc)                      !Guo
!   ty=dt*(twopi**2)*xnu/(2.d0*sc)                      !Guo 
!   w=twopi*dx                                          !Guo
!   tx = tx * &                                         !Guo
!        ( (2.d0*asixt*dx2*(1.d0-cos(w)) + &            !Guo
!           2.d0*bsixt*dx2*(1.d0-cos(2.d0*w)) + &       !Guo
!           2.d0*csixt*dx2*(1.d0-cos(3.d0*w)) &         !Guo
!           ) / (1.d0+2.d0*alsaixt*cos(w)) &            !Guo
!         )/(w**2)                                      !Guo
!   w=twopi*dy                                          !Guo
!   ty = ty * &                                         !Guo
!        ( (2.d0*asjyt*dy2*(1.d0-cos(w)) + &            !Guo
!           2.d0*bsjyt*dy2*(1.d0-cos(2.d0*w)) + &       !Guo
!           2.d0*csjyt*dy2*(1.d0-cos(3.d0*w)) &         !Guo
!           ) / (1.d0+2.d0*alsajyt*cos(w)) &            !Guo
!         )/(w**2)                                      !Guo  
!   if (iimplicit.eq.0) then                            !Guo
!      tc=sqrt( 9.d0*((tx+ty)**2)-2.d0*(tx+ty)+1.d0 )   !Guo
!      tc=tc-3.d0*tx-3.d0*ty+1.d0                       !Guo
!      tc=tc/2.d0                                       !Guo
!      phis1=-2.d0*(tx+ty)*phi1/tc/dt                   !Guo
!   else                                                !Guo 
!      if (istret.eq.0) then                            !Guo
!         tc=sqrt( (3.d0*tx+ty-1)**2+4.d0*tx*(1.d0+ty) ) !Guo
!         tc=tc-3.d0*tx-ty+1.d0                          !Guo 
!         tc=tc/2.d0/(ty+1.d0)                           !Guo
!         phis1=-2.d0*tx*phi1/tc/dt                      !Guo
!      else                                              !Guo
!         do k=1,xsize(3)                                !Guo
!         do j=1,xsize(2)                                !Guo 
!         do i=1,xsize(1)                                !Guo 
!            phis1(i,j,k)=xnu*( -(twopi**2)*phi1(i,j,k)-pp4y(j)*uz1(i,j,3) )/sc !Guo
!         enddo                                                                 !Guo 
!         enddo                                                                 !Guo
!         enddo                                          !Guo
!         phis1=phis1*(1.d0+6.d0*tx)/(1.d0+2.d0*tx)      !Guo
!         ilast=ifirst                                   !Guo
!      endif                                             !Guo
!   endif                                                !Guo
!   !                                                    !Guo
!   ! Velocity                                           !Guo
!   tx=dt*(twopi**2)*xnu/2.d0                            !Guo 
!   ty=dt*(twopi**2)*xnu/2.d0                            !Guo
!   w=twopi*dx                                           !Guo
!   tx = tx * &                                          !Guo
!        ( (2.d0*asix*dx2*(1.d0-cos(w)) + &              !Guo
!           2.d0*bsix*dx2*(1.d0-cos(2.d0*w)) + &         !Guo
!           2.d0*csix*dx2*(1.d0-cos(3.d0*w)) &           !Guo
!           ) / (1.d0+2.d0*alsaix*cos(w)) &              !Guo
!         )/(w**2)                                       !Guo
!   w=twopi*dy                                           !Guo 
!   ty = ty * &                                          !Guo 
!        ( (2.d0*asjy*dy2*(1.d0-cos(w)) + &              !Guo
!           2.d0*bsjy*dy2*(1.d0-cos(2.d0*w)) + &         !Guo
!           2.d0*csjy*dy2*(1.d0-cos(3.d0*w)) &           !Guo
!           ) / (1.d0+2.d0*alsajy*cos(w)) &              !Guo 
!        )/(w**2)                                       !Guo 
!   if (iimplicit.eq.0) then                             !Guo
!      tc=sqrt( 9.d0*((tx+ty)**2)-2.d0*(tx+ty)+1.d0 )    !Guo
!      tc=tc-3.d0*tx-3.d0*ty+1.d0                        !Guo
!      tc=tc/2.d0                                        !Guo 
!      gx1=-2.d0*(tx+ty)*gx1/tc/dt                       !Guo
!      gy1=-2.d0*(tx+ty)*gy1/tc/dt                       !Guo
!   else                                                 !Guo 
!      if (istret.eq.0) then                             !Guo
!         tc=sqrt( (3.d0*tx+ty-1)**2+4.d0*tx*(1.d0+ty) )  !Guo
!         tc=tc-3.d0*tx-ty+1.d0                           !Guo
!         tc=tc/2.d0/(ty+1.d0)                            !Guo
!         gx1=-2.d0*tx*gx1/tc/dt                          !Guo
!         gy1=-2.d0*tx*gy1/tc/dt                          !Guo
!      else                                               !Guo
!         do k=1,xsize(3)                                 !Guo
!         do j=1,xsize(2)                                 !Guo 
!         do i=1,xsize(1)                                 !Guo
!            gx1(i,j,k)=xnu*( -(twopi**2)*ux1(i,j,k)-pp4y(j)*uz1(i,j,1) )   !Guo
!            gy1(i,j,k)=xnu*( -(twopi**2)*uy1(i,j,k)-pp4y(j)*uz1(i,j,2) )   !Guo
!         enddo                                                             !Guo 
!         enddo                                                             !Guo 
!         enddo                                                             !Guo
!         gx1=gx1*(1.d0+6.d0*tx)/(1.d0+2.d0*tx)                             !Guo
!         gy1=gy1*(1.d0+6.d0*tx)/(1.d0+2.d0*tx)                             !Guo
!         ilast=ifirst                                                      !Guo 
!      endif                                                                !Guo
!   endif                                                                   !Guo
!   uz1=0.d0                                              !Guo      
...
!   hz1=0.d0                                              !Guo
   !                                                     !Guo 
!   ! Avoid explicit Euler time scheme at first time step  !Guo
!   ilit=1                                                 !Guo 
!endif                                                     !Guo
