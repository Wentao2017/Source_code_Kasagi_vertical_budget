...
if (iimplicit==0) then ! EXPLICITE !Guo
!-->for ux
if (istret.ne.0) then 
...
if (istret.ne.0) then                                                          !Guo

!-->for ux                                                                     !Guo
   call dery (te2,ux2,di2,sy,ffyp,fsyp,fwyp,ppy,ysize(1),ysize(2),ysize(3),1)  !Guo                                       
   do k=1,ysize(3)                                                             !Guo                                       
   do j=1,ysize(2)                                                             !Guo                                       
   do i=1,ysize(1)                                                             !Guo
      td2(i,j,k)=-pp4y(j)*te2(i,j,k)                                           !Guo                     
   enddo                                                                       !Guo                                       
   enddo                                                                       !Guo                                       
   enddo                                                                       !Guo                                       
!-->for uy                                                                     !Guo
   call dery (tf2,uy2,di2,sy,ffy,fsy,fwy,ppy,ysize(1),ysize(2),ysize(3),0)     !Guo                                       
   do k=1,ysize(3)                                                             !Guo                                       
   do j=1,ysize(2)                                                             !Guo                                       
   do i=1,ysize(1)                                                             !Guo
      te2(i,j,k)=-pp4y(j)*tf2(i,j,k)                                           !Guo                     
   enddo                                                                       !Guo                                       
   enddo                                                                       !Guo                                       
   enddo                                                                       !Guo                                       
!-->for uz                                                                     !Guo
   call dery (tj2,uz2,di2,sy,ffyp,fsyp,fwyp,ppy,ysize(1),ysize(2),ysize(3),1)  !Guo                                       
   do k=1,ysize(3)                                                             !Guo                                       
   do j=1,ysize(2)                                                             !Guo                                       
   do i=1,ysize(1)                                                             !Guo
      tf2(i,j,k)=-pp4y(j)*tj2(i,j,k)                                           !Guo                     
   enddo                                                                       !Guo                                       
   enddo                                                                       !Guo                                       
   enddo                                                                       !Guo                                       
...
!find j=1 and j=ny
      ...
      !print *,'amore',nrank,xstart(2),ny-(nym/p_row),nym,dims(1) !
...
!call derxx (ta1,phi1,di1,sx,sfxp,ssxp,swxp,xsize(1),xsize(2),xsize(3),1)            !Guo
call derxxt (ta1,phi1,di1,sx,sfxt,ssxt,swxt,xsize(1),xsize(2),xsize(3),0) ! npaire=0 !Guo
...
if (istret.ne.0) then 
 ! call deryy (ta2,phi2,di2,sy,sfyp,ssyp,swyp,ysize(1),ysize(2),ysize(3),1)            !Guo
   call deryyt(ta2,phi2,di2,sy,sfyt,ssyt,swyt,ysize(1),ysize(2),ysize(3),0) ! npaire=0 !Guo
...
!call derzz (ta3,phi3,di3,sz,sfzp,sszp,swzp,zsize(1),zsize(2),zsize(3),1)             !Guo
call derzzt (ta3,phi3,di3,sz,sfzt,sszt,swzt,zsize(1),zsize(2),zsize(3),0) ! npaire=0  !Guo
...
!if (nrank==0) print *,'Not ready'  !Guo
!stop                               !Guo 
   if (adt(itr)==0.) then                                            !Guo
      do ijk=1,nxyz                                                  !Guo 
         phis1(ijk,1,1)=dt*ta1(ijk,1,1)                              !Guo
      enddo                                                          !Guo
   else                                                              !Guo
      do ijk=1,nxyz                                                  !Guo
         phis1(ijk,1,1)=adt(itr)*phis1(ijk,1,1)+dt*ta1(ijk,1,1)      !Guo
      enddo                                                          !Guo
   endif                                                             !Guo
   do ijk=1,nxyz                                                     !Guo
      phi1(ijk,1,1)=phi1(ijk,1,1)+bdt(itr)*phis1(ijk,1,1)            !Guo
   enddo                                                             !Guo
endif
