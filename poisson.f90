...
do i = sp%yst(1), sp%yen(1)
   xtt=cmplx((bicix6*2.*cos(real(exs(i))*3.*dx/2.)+&
        cicix6*2.*cos(real(exs(i), kind=mytype)*5.*dx/2.)+&          !Guo
        dicix6*2.*cos(real(exs(i), kind=mytype)*7.*dx/2.)),&         !Guo 
        (bicix6*2.*cos(real(exs(i), kind=mytype)*3.*dx/2.)+&
        cicix6*2.*cos(real(exs(i), kind=mytype)*5.*dx/2.)+&          !Guo
        dicix6*2.*cos(real(exs(i), kind=mytype)*7.*dx/2.)), kind=mytype)  !Guo

   ytt=cmplx((biciy6*2.*cos(real(eys(j), kind=mytype)*3.*dy/2.)+&
        ciciy6*2.*cos(real(eys(j), kind=mytype)*5.*dy/2.)+&           !Guo 
        diciy6*2.*cos(real(eys(j), kind=mytype)*7.*dy/2.)),&          !Guo
        (biciy6*2.*cos(real(eys(j), kind=mytype)*3.*dy/2.)+&
        ciciy6*2.*cos(real(eys(j), kind=mytype)*5.*dy/2.)+&           !Guo
        diciy6*2.*cos(real(eys(j), kind=mytype)*7.*dy/2.)), kind=mytype)   !Guo

   ztt=cmplx((biciz6*2.*cos(real(ezs(k), kind=mytype)*3.*dz/2.)+&
        ciciz6*2.*cos(real(ezs(k), kind=mytype)*5.*dz/2.)+&            !Guo 
        diciz6*2.*cos(real(ezs(k), kind=mytype)*7.*dz/2.)),&           !Guo
        (biciz6*2.*cos(real(ezs(k), kind=mytype)*3.*dz/2.)+&
        ciciz6*2.*cos(real(ezs(k), kind=mytype)*5.*dz/2.)+&            !Guo
        diciz6*2.*cos(real(ezs(k), kind=mytype)*7.*dz/2.)), kind=mytype)  !Guo
...
!   print *,i,j,k, kxyz(i,j,k)
      ...
      do i = sp%xst(1),sp%xen(1)  
         ...
         ztt=cmplx((biciz6*2.*cos(real(ezs(k), kind=mytype)*3.*dz/2.)+&
              ...
              ciciz6*2.*cos(aimag(ezs(k))*5.*dz/2.)+&                        !Guo
              diciz6*2.*cos(aimag(ezs(k))*7.*dz/2.)), kind=mytype)           !Guo
